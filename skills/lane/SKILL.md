...
4. Only after the matrix is clean and consensus is ack'd: `lane validate` -> `lane advance --phase 4_verify` -> commit/push/PR, in that order, with no further edits to spec.md/verification.yaml/critic.yaml afterward (an edit after ack invalidates the digest binding).
